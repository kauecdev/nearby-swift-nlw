...
//  nlw-pocket-swift
//
//  Created by Kauê Cavalcante on 11/12/24.
//
    ...
    func fetchInitialData(completion: @escaping ([Category]) -> Void) {
        fetchCategories { categories in
            ...
            if let foodCategory = categories.first(where: { $0.name == "Alimentação" }) {
                self.fetchPlaces(for: foodCategory.id, userLocation: CLLocationCoordinate2D(
                    latitude: self.userLatitude, longitude: self.userLongitude))
            }
        }
    }
    ...
    func fetchCategories(completion: @escaping ([Category]) -> Void) {
        ...
        URLSession.shared.dataTask(with: url) { data, _, error in
            ...
            do {
                ...
                print("Failed to decode categories from data")
                completion([])
            }
        }.resume()
    }
    ...
    func fetchPlaces(for categoryID: String, userLocation: CLLocationCoordinate2D) {
        ...
        URLSession.shared.dataTask(with: url) { data, _, error in
            ...
            do {
                ...
                print("Failed to decode places from data: ", error)
            }
        }.resume()
    }
}
